...
function ClientLoad(json_obj) {
    ...
    $.each(opElement, function (index, opEl) {
        ...
        if (opEl.type == 'btn') {
            ...
            // associa eventi ajax / registrazione eventi per invio dei comandi generati dagli oggetti
            ...
            $el.slider({
                ...
                stop: function () {
                    ...
                    if (opEl.canSendEvents) {
                    	if (opEl.id == 'i13'){
                    		$('#lbl_delay').html($el.slider("value") + ' ms');
            ...
            $(document.body).css({ 'background-color': opEl.background });
            $("#page").css({ 'width': opEl.width });
        }
    });
...
function updateStatus(xmldoc, status, request) {
    ...
    // status_v2.xml
    /*
    $(xml).find('btn').each(function () {
    var $el = $(this);
    var i = parseInt0($el.find('i').text());
    var v = $el.find('v').text();
    XmlFoundBtn(i, v);
    });

    $(xml).find('sld').each(function () {
    var $el = $(this);
    var i = parseInt0($el.find('i').text());
    var v = parseInt0($el.find('v').text());
    XmlFoundSld(i, v);
    });
...
function XmlFoundLabel(i, v) {
    if (i < 0) return;
    //var opList = opGetElementsByType('label', i);
    ...
    if (i == 0) {
    	...
    	if (v == 0){
    	     $("#lbl_touche").html('');
    	     $("#fullColorBtn").css("bgcolor","#F36F05");
	}
    	else if (v == 1){
    	     $("#lbl_touche").html('Contact!');
    	     $("#fullColorBtn").css("bgcolor","#C0C0C0");
    	}
    }
    else if (i == 1) {
    	$("#fullColorBtn").css("bgcolor","#EE178C");
    	console.log('Changing color')
    }
    else if (i == 2) {
    	$("#i16").slider({ value: v });
    	$("#lbl_ultra_son").html(v + ' cm');
    }
...
function ajaxCall(s, fn, repeat_ms) {
    //setTimeout(function () { ajaxCall2(s, fn, repeat_ms); }, 50);
